    ...
    switch (step.key) {
      ...
      case "deliver": {
        if (job.config) {
          ...
          // hand over the generated code to disk (per-user local git repo), NOT compiled
          const localDir = await deliverToDisk(job)
          if (localDir) { job.artifacts.localPath = localDir; step.detail = `entregado: ${localDir.split("/").slice(-2).join("/")}` }
          if (githubConfigured()) {
            const bp = job.artifacts.blueprint
...
/** Deliver the GENERATED code to disk as a local git repo, organized per user with an
 *  autoincrement number (so repeated names don't clash): generated/projects/<user>/<NNN-slug>/.
 *  NOT compiled — just handed over; the user compiles+exports later (BYO GitHub/Vercel). */
async function deliverToDisk(job: JobRow): Promise<string | null> {
  try {
    if (!job.config) return null
    const { rows } = await query<{ user_email: string | null }>(`SELECT user_email FROM puglit_jobs WHERE id=$1`, [job.id])
    const root = process.env.PUGLIT_PROJECTS_DIR || join(process.cwd(), "..", "generated", "projects")
    const userDir = String(rows[0]?.user_email || "anon").replace(/[^a-zA-Z0-9._-]/g, "_")
    const base = join(root, userDir)
    mkdirSync(base, { recursive: true })
    const n = readdirSync(base).filter((d) => /^\d{3}-/.test(d)).length + 1
    const dir = join(base, `${String(n).padStart(3, "0")}-${job.slug}`)
    mkdirSync(dir, { recursive: true })
    const files = [
      { path: "domain.config.ts", content: configToTs(job.config) },
      { path: ".env.example", content: job.artifacts.envFile || "" },
      { path: "sql/app.sql", content: job.artifacts.sql || "" },
      { path: "sql/seed.sql", content: job.artifacts.seedSql || "" },
      { path: "README.md", content: `# ${job.name}\n\n${job.artifacts.blueprint?.summary || ""}\n\n_Generado por Puglit — código entregado SIN compilar. Usá "Compilar y exportar" para publicarlo (GitHub/Vercel)._\n` },
      ...((job.artifacts.appFiles || []) as { path: string; content: string }[]),
    ].filter((f) => f.content)
    for (const f of files) { const fp = join(dir, f.path); mkdirSync(dirname(fp), { recursive: true }); writeFileSync(fp, f.content) }
    try { execSync('git init -q && printf "node_modules\\n.next\\n.env.local\\n" > .gitignore && git add -A && git -c user.email=puglit@puglit.app -c user.name=Puglit commit -qm "Generated by Puglit"', { cwd: dir, stdio: "ignore" }) } catch { /* git optional */ }
    return dir
  } catch (e) { console.error("[deliverToDisk]", (e as Error).message); return null }
}
