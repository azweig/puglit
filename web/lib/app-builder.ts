...
- A BOOKING / RENTAL marketplace (Airbnb/hotel/coworking) ⇒ tables: listings (host_id, title, address, lat, lng, city, capacity, nightly_price_cents, cleaning_fee_cents, weekly_discount_pct, min_nights, max_nights, cancellation_policy ENUM[flexible|moderate|strict], status ENUM[draft|published|paused], instant_book), listing_photos, listing_amenities (join), availability_blocks (listing_id, start_date, end_date), bookings (listing_id, guest_id, check_in DATE, check_out DATE, guests_count, nights, base_cents, cleaning_cents, service_fee_cents, taxes_cents, total_cents, status ENUM[requested|accepted|declined|confirmed|cancelled|completed], cancellation_policy_snapshot), payments (booking_id, kind ENUM[hold|charge|refund|payout], amount_cents, status), reviews (booking_id, author_id, target_kind, rating, body, is_visible), messages (booking_id, sender_id, body). MONEY IN INTEGER CENTS ONLY (never floats). The HARD domain logic — availability/overlap, anti-double-booking, deterministic pricing (search total === checkout total), refund-by-policy-snapshot, double-blind reviews — is provided PRE-BUILT in lib/rentals/{pricing,availability,booking,refund,reviews}.ts: IMPORT and use those (priceQuote, isAvailable, createBooking, transition, cancelBooking, submitReview) instead of re-implementing them; the anti-double-booking DB constraint is added automatically. Your job: the listings CRUD + photos, the search route (filter by city/dates/guests/amenities, paginate, price each result with priceQuote so it matches checkout), host onboarding, messaging, and the UI pages. If the brief asks for verification, ALSO emit scripts/verify.sh that boots the app, hits /api/health, seeds data, and runs the adversarial scenarios (concurrency, pricing-equality, refund, etc.) against your real endpoints.
