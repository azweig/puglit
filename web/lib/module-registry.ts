...
export const BUILTIN_MODULES: Module[] = [
  ...
  // ── swarm-harvested utils (built + promoted during genetic training; re-adapted for the web) ──
  { name: "slugify", category: "util", description: "URL-friendly slugs, zero-dep: slugify() + uniqueSlugify() (content-hashed, collision-resistant). For pretty permalinks.", whenToUse: "the product has public pages/blog/listings/profiles with readable URLs", envVars: [] },
  { name: "pagination", category: "util", description: "Keyset (cursor) pagination, zero-dep: paginate() — fast on large tables, no OFFSET scan, parameterized values. Returns {data,nextCursor}.", whenToUse: "the product has feeds/timelines/long lists/infinite scroll", envVars: [] },
  { name: "csv-export", category: "util", description: "Rows → CSV, zero-dep: toCsv() (RFC-4180 quoting) + csvResponse() (a downloadable Response straight from a route).", whenToUse: "the product exports/downloads data or reports", envVars: [] },
  { name: "ics-calendar", category: "util", description: "Build .ics calendar files, zero-dep: buildIcs() + icsResponse() (downloadable Response). For 'add to calendar' / event export.", whenToUse: "the product has events/bookings/appointments/schedules", envVars: [] },
]
