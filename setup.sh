...
if yn "Configure an IMAGE provider for real logos/images (else a text monogram is used)?" n; then
  echo "    Examples: local Flux (run infra/setup-flux.sh, then http://localhost:8188/v1), DALL-E (https://api.openai.com/v1), fal/replicate proxy."
  IMG_URL="$(ask 'Images base URL' 'http://localhost:8188/v1')"
  IMG_MODEL="$(ask 'Image model' 'flux.1-schnell')"
  read -r -s -p "    Image API key (blank if local): " IMG_KEY; echo
  IMG_PROVIDER="compat"; ok "image provider: $IMG_MODEL @ $IMG_URL"
fi

# ── 5. Generate .env.local ─────────────────────────────────────────────────────
say "5/7  Generating $ENV_FILE"
mkdir -p "$WEB"
setkv(){ if grep -q "^$1=" "$ENV_FILE" 2>/dev/null; then sed -i "s|^$1=.*|$1=$2|" "$ENV_FILE"; else echo "$1=$2" >> "$ENV_FILE"; fi; }
if [ -f "$ENV_FILE" ] && ! yn ".env.local already exists. Overwrite?" n; then
  warn "Keeping .env.local — but FORCE-FIXING the model tiers (a stale small-model config is what makes output generic)"
  if [ "$PROVIDER" != "api" ]; then
    [ -n "$M_PREMIUM" ]  && setkv PUGLIT_MODEL_PREMIUM  "$M_PREMIUM"
  ...
  if [ -n "$IMG_PROVIDER" ]; then setkv PUGLIT_IMAGE_PROVIDER "$IMG_PROVIDER"; setkv PUGLIT_IMAGE_URL "$IMG_URL"; setkv PUGLIT_IMAGE_MODEL "$IMG_MODEL"; [ -n "$IMG_KEY" ] && setkv PUGLIT_IMAGE_KEY "$IMG_KEY"; fi
else
  {
    ...
    if [ -n "$IMG_PROVIDER" ]; then
      echo "PUGLIT_IMAGE_PROVIDER=$IMG_PROVIDER"; echo "PUGLIT_IMAGE_URL=$IMG_URL"; echo "PUGLIT_IMAGE_MODEL=$IMG_MODEL"
      [ -n "$IMG_KEY" ] && echo "PUGLIT_IMAGE_KEY=$IMG_KEY"
    fi
